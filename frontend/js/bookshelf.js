...
(function () {
  ...
  async function render() {
    ...
    // Loading state
    grid.innerHTML = '<p style="text-align:center;padding:2rem;color:var(--text-muted)">Loading your library…</p>';

    if (!window.MemoriumAPI || !window.MemoriumAPI.isAuthed()) {
      grid.innerHTML = '<p style="text-align:center;padding:2rem">Please <a href="login.html" style="text-decoration:underline">sign in</a> to see your journals.</p>';
      return;
    }
    ...
    try {
      ...
      journals.forEach(j => {
        ...
        card.className = 'card';
        card.style.cssText = 'aspect-ratio:3/4;margin-bottom:var(--space-6);cursor:pointer;background:var(--paper);border:1px solid var(--border);border-radius:12px;overflow:hidden;display:flex;flex-direction:column';
  ...
  function bindCreate(scope) {
    const btn = scope ? scope.querySelector('#create-first') : document.querySelector('#create-first, a[href="journal.html"]');
  ...
  function bindMigrate() {
    ...
    b.addEventListener('click', async () => {
      ...
      if (!local || !local.pages.length) { alert('No local data to migrate'); return; }
      // Need a journal to migrate into
      let jid = localStorage.getItem('memorium-current-journal');
      if (!jid) {
        // create one
        try {
          const res = await window.MemoriumAPI.createJournal({ title: 'Migrated Journal', description: 'From localStorage' });
          jid = res.data._id;
        } catch (e) { alert('Create journal failed: ' + e.message); return; }
      }
